...
// ইম্পোর্ট পাথ থেকে .tsx মুছে ফেলা হয়েছে
...
const LoadingScreen: React.FC = () => {
  return (
    <div className="fixed inset-0 z-[200] bg-slate-100 flex flex-col items-center p-6 text-center">
      <div className="flex-1 flex flex-col items-center justify-center w-full max-w-sm">
        <div className="relative mb-8">
          <div className="w-32 h-32 md:w-40 md:h-40 bg-white rounded-[2rem] shadow-2xl border border-slate-200/50 flex items-center justify-center p-5 overflow-hidden">
        ...
        <div className="space-y-4 w-full">
          <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-tighter uppercase">Microbus Schedule</h1>
...
const App: React.FC = () => {
  ...
  const handleSaveBooking = async (booking: Booking) => {
    try {
      ...
      await set(ref(db, `bookings/${id}`), bookingToSave);
      closeBookingModal();
    } catch (e) {
  ...
  return (
    <>
      {isLoading ? (
        ...
        <div className="flex flex-col bg-slate-100 text-slate-900 h-[100dvh] overflow-hidden">
          <header className="bg-white border-b border-slate-200 px-2 md:px-6 py-1 md:py-1.5 grid grid-cols-[1fr_auto_1fr] items-center sticky top-0 z-50 shadow-sm shrink-0">
            <section className="flex justify-start">
              {view === 'reports' ? (
                <button 
                  ...
                  className="w-10 h-10 md:w-14 md:h-14 flex items-center justify-center bg-slate-100 text-slate-600 rounded-lg md:rounded-xl hover:bg-slate-900 hover:text-white transition-all shadow-sm"
                >
            ...
            <div className="text-center px-2 flex flex-col justify-center">
              <h1 className="text-[14px] sm:text-lg md:text-[24px] font-black text-slate-900 tracking-tight uppercase whitespace-nowrap">
                {view === 'reports' ? 'Report Center' : 'MICROBUS SCHEDULE'}
              </h1>
              <p className="text-[9px] sm:text-[10px] md:text-[12px] font-bold text-indigo-600 tracking-[0.1em] md:tracking-[0.2em] uppercase mt-0.5 whitespace-nowrap">
            ...
            <div className="flex items-center justify-end gap-2">
              {isAdmin && (
                <div className="flex items-center gap-2">
                  <span className="hidden sm:inline text-indigo-700 text-[9px] font-bold uppercase bg-indigo-50 px-2 py-1 rounded-md border border-indigo-100">Admin</span>
                  <button onClick={handleLogout} className="text-slate-400 hover:text-red-600 p-1.5 md:p-2 bg-slate-50 rounded-lg shadow-sm">
                    <LogOut size={16} />
                  </button>
                </div>
              )}
          ...
          <main className="p-2 md:p-4 flex flex-col gap-2 md:gap-4 flex-1 overflow-hidden">
            {view === 'calendar' ? (
              <div className="flex flex-col gap-2 md:gap-4 h-full">
                <div className="bg-white rounded-2xl md:rounded-3xl shadow-xl border border-slate-200 overflow-hidden flex flex-col flex-1">
                  <Calendar 
                ...
                {isAdmin && (
                  <div className="flex justify-center shrink-0 pb-1">
                    <button 
                      ...
                      className="flex items-center gap-2 md:gap-4 bg-indigo-600 text-white px-6 md:px-10 py-3 md:py-4 rounded-xl md:rounded-2xl font-black uppercase tracking-widest shadow-lg hover:bg-indigo-700 transition-all text-[10px] md:text-sm"
                    >
              ...
              <div className="max-w-5xl mx-auto w-full h-full overflow-y-auto custom-scrollbar">
