...
const MasterSettingsModal: React.FC<MasterSettingsModalProps> = ({ 
  ...
  const inputClasses = "w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm text-off-white outline-none focus:border-emerald-500 transition-all placeholder:text-silver/30 shadow-inner";
  const labelClasses = "text-[10px] font-black text-silver uppercase tracking-widest mb-1.5 ml-1 block";
  ...
  return (
    <div className="flex flex-col h-full max-h-[85vh] animate-in fade-in duration-500 overflow-hidden relative">
      <div className="flex items-center gap-1 md:gap-2 overflow-x-auto custom-scrollbar pb-3 border-b border-white/10 shrink-0">
        {tabs.map(tab => (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-xl font-black uppercase text-[9px] md:text-[10px] tracking-widest transition-all whitespace-nowrap border ${activeTab === tab.id ? 'bg-emerald-600 border-emerald-500 text-off-white shadow-lg' : 'bg-white/5 border-white/5 text-silver hover:text-off-white hover:bg-white/10'}`}
          >
            <tab.icon size={14} />
            {tab.label}
          </button>
              ...
              <div className={cardClasses}>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <button 
                    ...
                    className="w-8 h-8 bg-emerald-600 hover:bg-emerald-500 text-off-white rounded-lg flex items-center justify-center shadow-lg transition-all active:scale-90"
                  ...
                  {(formData.branding.footerLines || []).map((line, idx) => (
                    <div key={idx} className="flex gap-2 animate-in slide-in-from-right-2 duration-200">
                      <div className="flex-1 relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-silver">{idx + 1}</span>
                        <input 
                          type="text" 
                          value={line} 
                      ...
                      <button 
                        ...
                        className="w-10 h-10 bg-rose-600/20 hover:bg-rose-600 text-rose-500 hover:text-off-white rounded-xl flex items-center justify-center transition-all active:scale-90 shrink-0 border border-rose-500/20"
                      >
                        <Trash2 size={16} />
                      </button>
                    </div>
            ...
            <div className="space-y-4">
              <div className={cardClasses}>
                ...
                <div className="space-y-10 px-2">
                  <div className="space-y-3">
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-silver">
                      <span>Watermark Transparency</span>
                      <span className="text-emerald-500">{Math.round(formData.ui.watermarkOpacity * 100)}%</span>
                    </div>
                    <input type="range" min="0" max="0.5" step="0.01" value={formData.ui.watermarkOpacity} onChange={e => handleUpdate('ui', 'watermarkOpacity', Number(e.target.value))} className="w-full h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <input type="range" min="0" max="0.3" step="0.01" value={formData.ui.gridOpacity} onChange={e => handleUpdate('ui', 'gridOpacity', Number(e.target.value))} className="w-full h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div className="space-y-3">
                      ...
                      <input type="range" min="0.1" max="1" step="0.1" value={formData.ui.glassIntensity} onChange={e => handleUpdate('ui', 'glassIntensity', Number(e.target.value))} className="w-full h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
                    </div>
                    <div className="space-y-3">
                      <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-silver">
                      ...
                      <input type="range" min="0" max="40" step="2" value={formData.ui.borderRadius} onChange={e => handleUpdate('ui', 'borderRadius', Number(e.target.value))} className="w-full h-1.5 bg-white/10 rounded-lg appearance-none cursor-pointer accent-emerald-500" />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="relative">
                        <Hash className="absolute left-4 top-1/2 -translate-y-1/2 text-silver" size={14} />
                      ...
                      <p className="text-[8px] font-bold text-silver/50 uppercase mt-2 ml-1">Daily operations access code</p>
                    </div>
                    <div>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="relative">
                        <Clock className="absolute left-4 top-1/2 -translate-y-1/2 text-silver" size={14} />
                    ...
                    <div className="pt-2 space-y-3">
                      <div className="space-y-3">
                        <button 
                          ...
                          className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all ${formData.security.maintenanceMode ? 'bg-amber-600/20 border-amber-500 shadow-xl' : 'bg-white/5 border-white/10 text-silver'}`}
                        >
                          <div className="flex items-center gap-3">
                            <ShieldAlert size={18} className={formData.security.maintenanceMode ? 'text-amber-500' : 'text-silver/40'} />
                            <span className="text-[10px] font-black uppercase tracking-widest">System Lockdown</span>
                          </div>
                          <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${formData.security.maintenanceMode ? 'bg-amber-500 border-amber-400' : 'border-white/10'}`}>
                            {formData.security.maintenanceMode && <Check size={12} className="text-off-white" />}
                          </div>
                        </button>
                      ...
                      <button 
                        ...
                        className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all ${formData.security.maskPinInput ? 'bg-emerald-600/20 border-emerald-500 shadow-xl' : 'bg-white/5 border-white/10 text-silver'}`}
                      >
                        <div className="flex items-center gap-3">
                          <Monitor size={18} className={formData.security.maskPinInput ? 'text-emerald-500' : 'text-silver/40'} />
                          <span className="text-[10px] font-black uppercase tracking-widest">Input Masking</span>
                        </div>
                        <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${formData.security.maskPinInput ? 'bg-emerald-500 border-amber-400' : 'border-white/10'}`}>
                          {formData.security.maskPinInput && <Check size={12} className="text-off-white" />}
                        </div>
                      </button>
                    </div>
                  </div>
          ...
          {activeTab === 'system' && (
            <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                >
                  <div className="w-20 h-20 bg-blue-600 text-off-white rounded-2xl flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform">
                    <FileJson size={40} />
                  </div>
                  <div>
                    <p className="text-base font-black text-off-white uppercase tracking-tight">Generate Local Backup</p>
                    <p className="text-[10px] font-bold text-silver uppercase tracking-widest mt-1">Snapshot of all database records</p>
                  </div>
                </button>
                <div className="p-8 bg-white/5 border border-white/10 rounded-[2rem] flex flex-col items-center justify-center text-center gap-4 shadow-xl">
                  ...
                  <div>
                    <p className="text-[10px] font-black text-silver uppercase tracking-[0.3em]">Operational Health</p>
                    <p className="text-xl font-black text-off-white mt-1">SYSTEM ONLINE</p>
              ...
              <div className="p-8 bg-rose-950/20 rounded-[2.5rem] border-2 border-rose-900/40 space-y-8 shadow-2xl relative overflow-hidden">
                <div className="flex items-center gap-6">
                  <div className="w-16 h-16 bg-rose-600 text-off-white rounded-2xl flex items-center justify-center border border-rose-400/30 shadow-2xl shrink-0">
                    <Trash2 size={32} />
                  </div>
                  <div className="min-w-0">
                    <p className="text-lg font-black text-off-white uppercase tracking-tight">Factory State Reset</p>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 bg-gradient-to-t from-black/80 via-black/40 to-transparent shrink-0 z-40 pointer-events-none">
        <div className="flex justify-end pointer-events-auto">
          <button 
            onClick={() => onSave(formData)}
            className="px-14 py-5 bg-emerald-600 hover:bg-emerald-500 text-off-white rounded-2xl font-black uppercase text-xs tracking-[0.3em] shadow-[0_20px_50px_rgba(0,0,0,0.6)] active:scale-95 transition-all flex items-center gap-4 border border-emerald-400/30 ring-4 ring-black/20"
          >
            <Save size={20} />
