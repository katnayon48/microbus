...
const AttendanceViewer: React.FC<AttendanceViewerProps> = ({ isAdmin, onLoginClick, appSettings }) => {
  ...
  return (
    <div 
      className="relative h-full w-full rounded-2xl md:rounded-3xl p-3 md:p-4 border border-white/10 shadow-2xl overflow-hidden flex flex-col box-border glass-container"
    >
      {/* Watermark Logo */}
      ...
      <div className="relative z-10 flex-1 flex flex-col min-h-0 space-y-3 md:space-y-4">
        <div className="flex flex-col gap-2 md:gap-3 px-1 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 md:gap-3 min-w-0">
              ...
              <h4 className="text-[10px] md:text-base font-black text-off-white uppercase tracking-tight whitespace-nowrap truncate">ATTENDANCE LOG</h4>
            </div>
            <div className="bg-white/5 border border-white/10 px-2 py-1 rounded-full shrink-0">
              <span className="text-[7px] md:text-[9px] font-black text-off-white uppercase tracking-widest whitespace-nowrap">
          ...
          <div className="flex justify-center">
            <div className="flex items-center gap-1 md:gap-2 bg-white/5 backdrop-blur-md px-2 py-1 md:px-4 md:py-1.5 rounded-xl md:rounded-2xl border border-white/10 shadow-2xl shrink-0">
              <button 
                onClick={() => setHistoryMonth(m => subMonths(m, 1))} 
                className="p-1 text-silver hover:text-emerald-400 active:scale-90 transition-colors bg-white/5 rounded-lg"
              >
                <ChevronLeft size={14} className="md:w-5 md:h-5" />
              </button>
              <span className="text-[9px] md:text-[13px] font-black text-off-white uppercase tracking-[0.1em] md:tracking-[0.15em] min-w-[85px] md:min-w-[110px] text-center px-1">
        ...
        <div className="bg-white/5 rounded-xl md:rounded-[1.5rem] border border-white/10 overflow-hidden shadow-2xl w-full backdrop-blur-sm flex-1 flex flex-col min-h-0 box-border">
          {filteredAttendance.length > 0 ? (
                  ...
                  {filteredAttendance.map((record, index) => (
                    <tr key={record.id || index} className="group hover:bg-white/[0.04] transition-colors">
                      <td className="p-2 md:py-1.5 md:px-4 whitespace-nowrap border-r border-white/5 overflow-hidden">
                        <span className="text-[8.5px] md:text-sm font-black text-off-white uppercase tracking-tight">
                          {format(parseISO(record.date), 'dd MMM yy')}
                        </span>
                      </td>
                      <td className="p-2 md:py-1.5 md:px-4 whitespace-nowrap border-r border-white/5 overflow-hidden">
                        <span className="text-[7px] md:text-[10px] font-black text-silver/60 uppercase tracking-widest truncate block">
                      ...
                      {record.isHoliday && !record.isDutyDay ? (
                        ...
                        <>
                          <td className="p-2 md:py-1.5 md:px-4 text-center border-r border-white/5 overflow-hidden">
                            <span className="text-[8px] md:text-[13px] font-black text-off-white block truncate">{record.inTime || '--:--'}</span>
                          </td>
                          <td className="p-2 md:py-1.5 md:px-4 text-center border-r border-white/5 overflow-hidden">
                      ...
                      {!(record.isHoliday && !record.isDutyDay) && (
                        <>
                          ...
                          <td className="hidden md:table-cell py-1.5 px-4 overflow-hidden">
                            <span className="text-[11px] font-bold text-silver block truncate max-w-full" title={record.remarks}>
                              {record.remarks || '-'}
                            </span>
                          </td>
                        </>
                      ...
                      {record.isHoliday && !record.isDutyDay && (
                        <>
                          ...
                          <td className="hidden md:table-cell py-1.5 px-4">
                            <span className="text-[11px] font-bold text-silver italic block truncate">{record.remarks || '-'}</span>
                          </td>
                        </>
                      )}
                    </tr>
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-8">
              ...
              <p className="text-[9px] md:text-[12px] font-black uppercase tracking-[0.2em] text-silver text-center">No logs found</p>
            </div>
          )}
        </div>
        ...
        <div className="py-2 md:py-2 flex flex-col items-center gap-1.5 md:gap-2 shrink-0">
          <button 
            ...
            className="group flex items-center justify-center gap-2 px-6 py-2 md:px-10 md:py-2.5 bg-emerald-600 hover:bg-emerald-500 text-off-white rounded-xl md:rounded-2xl text-[9px] md:text-[11px] font-black uppercase tracking-widest shadow-xl transition-all active:scale-[0.98] border border-white/10"
          >
          ...
          <p className="text-[6px] md:text-[9px] font-bold text-silver uppercase tracking-[0.3em] opacity-50">Authorized Access Only</p>
        </div>
      </div>
    </div>
  );
